        ...
        <CardContent sx={{ p: 4 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
            <Box>
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 2, mb: 2 }}>
                {restaurant.phone ? (
                  <Chip
                    ...
                    color="info"
                    sx={{ fontWeight: 500, fontSize: 16, cursor: 'pointer' }}
                    onClick={() => alert(`Call: ${restaurant.phone}`)}
                  />
                ) : (
                  <Chip
                    icon={<span role="img" aria-label="phone">📞</span>}
                ...
                {restaurant.address && (
                  <Chip
                    icon={<span role="img" aria-label="address">📍</span>}
              ...
              {/* Cuisine, Rating, Payment Options */}
              <Box sx={{ display: 'flex', gap: 1, mb: 2, flexWrap: 'wrap' }}>
                ...
                {restaurant.supportsMobilePayment ? (
                  <Chip label="Mobile Payment" color="success" icon={<span role='img' aria-label='mobile'>📱</span>} sx={{ cursor: 'pointer' }} onClick={() => alert('Mobile Payment available!')} />
                ) : (
                  <Chip label="No Mobile Payment" color="default" icon={<span role='img' aria-label='mobile'>📱</span>} sx={{ opacity: 0.6 }} />
                )}
                {restaurant.supportsCOD ? (
                  <Chip label="Cash on Delivery" color="warning" icon={<span role='img' aria-label='cod'>💵</span>} sx={{ cursor: 'pointer' }} onClick={() => alert('Cash on Delivery available!')} />
                ) : (
                  <Chip label="No COD" color="default" icon={<span role='img' aria-label='cod'>💵</span>} sx={{ opacity: 0.6 }} />
                )}
              </Box>
            </Box>
            {/* REMOVE this block (if present) to avoid duplicate address/location: */}
